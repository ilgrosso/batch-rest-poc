...
public class BatchPayloadLineReader implements AutoCloseable {
    ...
    private boolean isBoundary(final String currentLine) {
        return (currentBoundary + BatchConstants.CRLF).equals(currentLine)
                || (currentBoundary + BatchConstants.DOUBLE_DASH + BatchConstants.CRLF).equals(currentLine);
    }
    ...
    private String readLine() throws IOException {
        ...
        if (innerBuffer.position() == 0) {
            ...
            if (currentLine.startsWith(HttpHeaders.CONTENT_TYPE)) {
                ...
                currentBoundary = BatchConstants.DOUBLE_DASH + multipartMixed.getParameters().
                        get(BatchConstants.BOUNDARY_PARAMETER);
            } else if (BatchConstants.CRLF.equals(currentLine)) {
